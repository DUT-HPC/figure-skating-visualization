  ...
  <el-container>
    <el-aside class="album-slide">
      ...
      <div class="album-info">
        <h2>基本资料</h2>
        <ul>
          <li>name：{{ songDetails.name }}</li>
          <li>age：{{ songDetails.age }}</li>
        </ul>
      </div>
    </el-aside>
...
import SongList from "@/components/VideoList.vue";
...
export default defineComponent({
  ...
  setup() {
    ...
    onMounted(async () => {
      try {
        const result = (await HttpManager.getVideoOfPlayerId(songDetails.value.id)) as ResponseBody;
